...
func get_block_label_text(value) -> String:
	var number = pow(2, value) as int
	var suffix = ""
	if value >= 10:
		suffix = "K"
		number = (number / 1024) as int
	elif value >= 20:
		suffix = "M"
		number = (number / 1048576) as int
	elif value >= 30:
		suffix = "T"
		number = (number / 1073741824) as int
